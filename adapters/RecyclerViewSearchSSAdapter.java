 ...
 * Date: 01/03/2017
 * Package: icn.premierandroid.adapters
 * Project Name: PremierAndroid
 */

public class RecyclerViewSearchSSAdapter extends RecyclerView.Adapter<RecyclerViewHolderAllFeeds> {

    private ArrayList<AllFeedsDataModel> latestUpdateList;
    ...
    public RecyclerViewSearchSSAdapter(Context context, ArrayList<AllFeedsDataModel> latestSearchUpdatesList) {
        this.latestUpdateList = latestSearchUpdatesList;
    ...
    public void onBindViewHolder(final RecyclerViewHolderAllFeeds holder, int position) {
        model = latestUpdateList.get(position);
...
//                    }
        ...
        Picasso.with(context).load(model.getImage()).noFade().into(holder.uploadedImage);
        ...
        for (int i = 0; i <= like_icons.size(); i++) {
            ...
            holder.number_of_comments.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    GoToComments(model.getImages(finalI), model.getCurrentUser());
                }
            });
                        ...
                        if (e == null) {
                            for (ParseObject j : objects) {
                                ...
                                if (likingArray != null) {
                                    ...
                                    if (liked && likingArray.toString().contains(ParseUser.getCurrentUser().getObjectId())) {
    ...
    public int getItemCount() {
        return (null != latestUpdateList ? latestUpdateList.size() : 0);
    }
    ...
    private void addLikeIcons(RecyclerViewHolderAllFeeds mainHolder) {
    ...
    public void update(ArrayList<AllFeedsDataModel> updatesList) {
        updatesList.clear();
        updatesList.addAll(updatesList);
    }
}
